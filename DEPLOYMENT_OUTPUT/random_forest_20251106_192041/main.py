...
app = FastAPI(title="Random Forest API Deployment"); pipeline = joblib.load('pipeline.pkl')
class PredictionInput(BaseModel): data: List[Dict[str, Any]]
@app.get("/")
def read_root(): return {"message": "Welcome to the Random Forest model API."}
@app.post("/predict")
def predict(payload: PredictionInput):
    try:
        df = pd.DataFrame(payload.data); predictions = pipeline.predict(df).tolist()
        if "regression" == 'classification' and hasattr(pipeline, "predict_proba"):
